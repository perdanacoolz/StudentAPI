...
namespace StudentAPI.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        private readonly IConfiguration _configuration; public AuthController(
        ...
        public async Task<IActionResult> RegisterAdmin([FromBody] RegisterModel model)
        {
            ...
            var result = await _userManager.CreateAsync(user, model.Password);
            if (!result.Succeeded)
                return StatusCode(StatusCodes.Status500InternalServerError, new Response { Status = "Error", Message = "User creation failed! Please check user details and try again." }); if (!await _roleManager.RoleExistsAsync(UserRoles.Admin))
                await _roleManager.CreateAsync(new IdentityRole(UserRoles.Admin));
            if (!await _roleManager.RoleExistsAsync(UserRoles.User))
                await _roleManager.CreateAsync(new IdentityRole(UserRoles.User)); if (await _roleManager.RoleExistsAsync(UserRoles.Admin))
            {
